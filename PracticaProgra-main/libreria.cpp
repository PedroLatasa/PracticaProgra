...
void operaciones(int eleccion, Cliente &c)
{
    int i=0;
    Cuenta &cta = c.cuentas.at(0); // si solo tiene una cuenta, no tendra que elegir (se coge directamente esa, es decir, la 0)
    if (c.cuentas.size() > 1 && eleccion != 0)
    {
        i=eligeCuenta(c);
        cta = c.cuentas.at(i); // eleccion del usuario sobre que cuenta quiere operar
    }
    cout << "\n\t sobre la " << i << " entonces\n";
    switch (eleccion)
    {
            ...
            if (archivo2.is_open())
            {
                ...
                for (int j = 0; j < numCuentas; j++)
                {
                    ...
                    if (cta.Nombre_Usuario == c.getDNI())
                    {
                        ...
                        cout << "\n\tAnnadido una cuenta a " << c.getDNI() << endl;
                    }
                }
                archivo2.close();
            }
...
int eligeCuenta(Cliente c)
{
    ...
    {
        cout << endl
             << i << "- " << c.cuentas.at(i).Numero_Tarjeta << endl;
    }
    cout << "Elige: ";
    cin >> eleccion;
